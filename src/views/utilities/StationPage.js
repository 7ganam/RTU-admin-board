...
const StationPage = () => {
    ...
    const clientRef = useRef(
        mqtt.connect('ws://ec2-52-28-144-120.eu-central-1.compute.amazonaws.com:8000', {
    ...
    useEffect(() => {
        ...
        clientRef.current.on('message', (topic, payload) => {
            const mystring = new TextDecoder().decode(payload);
            console.log('mystring', mystring);
            if (mystring) {
                const payloadArray = mystring.split(',');
                console.log('topic', topic);
                console.log('JSON.parse(payload)', payloadArray[0]);
                const myArray = ChartData1.series[0].data;
                myArray.shift();
            ...
            <Card sx={{ overflow: 'hidden' }}>
                <Grid container spacing={2}>
                    <Grid item xs={12} md={12} sx={{ pt: '16px !important' }}>
                        <BajajAreaChartCard title="RMS Voltage I" value="220v" ChartData1={ChartData1} />
                    </Grid>
                ...
                <Grid item xs={12}>
                    <Grid container spacing={gridSpacing} p={1}>
                        <Grid item sm={6} xs={12} md={3}>
                            <TotalIncomeLightCard isLoading={false} checked1={checked1} setchecked1={setchecked1} />
                        </Grid>
                        {/* <Grid item sm={6} xs={12} md={3}>
